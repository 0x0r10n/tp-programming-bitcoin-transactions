...
def op_checksig(stack, z):
    # Vérifie s'il y a au moins 2 éléments dans la pile
    if len(stack) < 2:
        return False
    # Récupère et retire le sommet de la pile, qui est la clé publique SEC
    sec_pubkey = stack.pop()
    # Récupère et retire la prochaine valeur de la pile, en supprimant le dernier octet (hash_type)
    der_sig = stack.pop()[:-1]
    try:
        # Tente de parser la clé publique sérialisée en tant que S256Point
        # et la signature DER en tant que Signature
        p = S256Point.parse(sec_pubkey)
        d = Signature.parse(der_sig)
    except (ValueError, SyntaxError):
        # En cas d'échec du parsing, retourne False
        return False
    # Vérifie si la signature correspond au hachage z
    if p.verify(z, d):
        # Si la vérification est réussie, ajoute 1 (True) à la pile
        stack.append(encode_num(1))
    else:
        # Sinon, ajoute 0 (False) à la pile
        stack.append(encode_num(0))
    # Retourne True pour indiquer que l'opération s'est déroulée avec succès
    return True
